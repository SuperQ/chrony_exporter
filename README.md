...
`--collector.clients` exports aggregate metrics from chronyd's client log, the
same data shown by `chronyc clients`. The metrics have no per-client labels, so
...
These metrics are not the same as the serverstats ones. The serverstats
counters are chronyd's own totals since startup and only go up, so use those
for long-term request rates. The clients hit and drop metrics are gauges, not
counters: they are a live sum over the clients in the log, so they rise as
clients are seen and fall again as old records are dropped. They are most
useful for the connected count and the last-seen and interval histograms.
...
When the log is full chronyd reuses the oldest records, so
`chrony_clients_connected` stops at that limit and the hit and drop gauges can
go down as clients are dropped. Increase `clientloglimit` to track more
clients, but note that it is real memory (up to 2 GB), so set it to match the
number of clients you expect.
